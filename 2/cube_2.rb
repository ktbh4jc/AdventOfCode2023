...
# A class that solves part 2 of https://adventofcode.com/2023/day/2
class Cube1
  ID_REGEX = /Game (?<id>\d+)/
  RED_REGEX = /(?<red>\d+) red/
  ...
  Game = Struct.new(:id, :pulls) do
    # Power is equal to the maximum number of each color of cube pulled multiplied together.
    #   For example: "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green" would need
    #   6 blue cubes, 4 red cubes, and 2 green cubes, so the power would be 6*4*2 = 48
    #
    # @return [Integer] equal to the power of a game
    def power
      highest_red = 0
      highest_blue = 0
      highest_green = 0
      pulls.each do |pull|
        highest_red = pull.red if pull.red > highest_red
        highest_blue = pull.blue if pull.blue > highest_blue
        highest_green = pull.green  if pull.green > highest_green
      end
  ...
  # Itterate over all games and sum their powers
  #
  # @return [Integer] the sum of all game powers
  def sum_game_power
    sum = 0
    @games.each do |game|
      sum += game.power
